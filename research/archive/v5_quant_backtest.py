...
async def run_v5_backtest(days=30):
    print(f"🚀 V5 HYPER-QUANT PORTFOLIO BACKTEST (Last {days} days)")
    print(f"Config: EMA_Trend={EMA_TREND}, ATR_Mult={ATR_MULTIPLIER}, Score>={MIN_CONFIDENCE_SCORE}")
    print(f"Filters: POC Value, Asian Quality ({ASIAN_RANGE_MIN_PIPS}pips), ADR {ADR_THRESHOLD_PERCENT*100}%")
    ...
    for t in timeline:
        ...
        for symbol in valid_symbols:
            ...
            # Scoring Engine V5.0
            score_details = {
                'h1_aligned': True, # Simple assumption for backtest
        ...
        for sig in filtered_batch:
            ...
            if hit == "WIN":
                ...
                if sig['at_value']: poc_wins += 1
            elif hit == "LOSS":
                total_losses += 1
            
            if hit:
                if sig['asian_sweep']: asian_sweep_trades += 1
                if sig['at_value']: poc_trades += 1
                cooldowns[symbol] = t + timedelta(hours=6)

    print("\n" + "═"*40)
    print(f"🏁 V5 HYPER-QUANT RESULTS")
    ...
    print(f"Volume POC Success: {poc_wins}/{poc_trades} ({(poc_wins/poc_trades*100) if poc_trades>0 else 0:.1f}%)")
    print("═"*40)

if __name__ == "__main__":
    asyncio.run(run_v5_backtest(30))
